...
int is_on_window(game_obj *g, scene *d)
{
    ...
    sfVector2f left_bottom = {left_top.x, left_top.y + (g->rect.height * scale.y)};
    sfVector2f right_top = {left_top.x + (g->rect.width * scale.x), left_top.y};
    sfVector2f right_bottom = {left_top.x + (g->rect.width * scale.x), left_top.y + (g->rect.height * scale.y)};
